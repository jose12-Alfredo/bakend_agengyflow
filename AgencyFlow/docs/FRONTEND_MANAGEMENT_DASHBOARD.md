...
periodo limita las métricas operativas mensuales mediante el solapamiento del
intervalo planificado (`startDate <= último día` y `endDate` nulo o
`endDate >= primer día`). El avance, en cambio, es acumulado: para los
proyectos y subproyectos activos en el mes considera todas sus tareas y
subtareas no eliminadas, incluidas las planificadas para fechas posteriores.
