...
public class DashboardService(AppDbContext db, ManagementDashboardService managementDashboardService)
{
    ...
    public async Task<DashboardDto> GetAsync(DashboardFilterDto? filter = null)
    {
        ...
        var parentTasks = await db.TaskItems
            .AsNoTracking()
            .Where(task =>
                task.DeletedAt == null &&
        ...
        var subTaskHistories = await db.SubTaskStatusHistories
            .AsNoTracking()
            .Where(history =>
        ...
        var taskHistories = await db.TaskStatusHistories
            .AsNoTracking()
            .Where(history =>
        ...
        var activeSubProjects = await db.SubProjects
            .AsNoTracking()
            .Where(subProject =>
            ...
            .Select(user =>
            {
                ...
                return new WorkloadItemDto
                {
                    ...
                    OverdueSubTaskCount = assignedSubTasks.Count(task =>
                        task.DaysOverdue > 0),
                    AverageProgressPercentage = assignedTasks.Count == 0
                        ? 0
                        : (int)Math.Round(
                            assignedTasks.Average(task =>
